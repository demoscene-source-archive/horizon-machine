...
const char fragment_glsl[] = ""
 ...
 "{"
   ...
   "float l=min(min(min(1-s(length(m.xz),abs(m.y)),s(length(m.xz),m.y)+.2),length(m-vec3(m.xz/length(m.xz)*.45,0).xzy)-.02),m.y+1);"
   "m=abs(m);"
   "if(m.z>m.x)m.xz=m.zx;"
   "return min(min(l,length(m-vec3(normalize(m.xy)*.45,0))-.02),a>120?length(m)-.4:100);"
 "}"
 "float s(vec3 m)"
 "{"
   "float i=min(min(m.y+.2,-(abs(length(m.xz)-1.85)-1.65-clamp(m.y-1,-1,0))),max(m.y-.15,2.5-length(m.xz)));"
 ...
 "{"
   ...
   "float l=min(s(length(m.xy)-.25,s(h(m,vec3(.2))-.5,abs(s(length(m.xz)-1,.6)))),h(m-vec3(0,a/5-11+7,4.7),vec3(1,8,.8)));"
   "m.z-=1;"
 ...
 "}"
 "vec3 g=c(a),w=c(x),r=(vec3(s(),s(),s())-.5)/256;"
 "vec3 e(vec3 m)"
 "{"
   "return vec3(-m.xy/m.z*vec2(.28125,.5)+.5,length(m)*32);"
 "}"
 ...
 "{"
   "vec4 z=vec4(0);"
   "vec2 x=vec2(mod(m.x/1.,2.)/2,mod(m.x/2.,2.)/2);"
   ...
   "if(m.w<1)"
     "{"
       ...
       "if(a<43||a>=75&&a<103)"
         "r.y+=sin(clamp((mod(f.y+.12,.05)/.05-.8)/.2*v,0.,v)),r=normalize(r);"
//       "r=normalize(r+vec3(0,sin(clamp((mod(f.y+.12,.05)/.05-.8)/.2*v,0.,v)),0));"
       "vec3 c=f,h=reflect(n,r),p=normalize(cross(r,r.zxy)),u=vec3(.9);"
       "z.w=length(f-g)/255*32;"
       "float F=1+8*step(.5,fract(atan(f.z-.8,f.x)/v*32));"
         ...
         " if(a<75)"
           "{"
             ...
             "if(f.y<-.5999&&(abs(abs(f.x)-.7)<.2||abs(abs(f.z)-.8)<.03))"
               "z.xyz=vec3(1,.4,.2)/4;"
             "if(abs(f.z)<1&&abs(abs(f.y)-.2)<.01&&a>50)"
                "z.xyz=vec3(1,.2,.1);"
             "F=1;"
             "u/=1.3;"
           "}"
       ...
       "f=c+r*.05;"
       "z.xyz+=e(f,h,F)*.75*clamp(1-dot(-n,r),0,1);"
       "for(int F=0;F++<4;)"
           "h.x=v*s(),h.y=s(),z.xyz+=e(f,(p*cos(h.x)+cross(r,p)*sin(h.x))*sqrt(h.y)+r*sqrt(1-h.y),7)/4*u*sqrt(1-h.y);"
       "f=w;"
       "y=.1;"
       ...
       "z.xyz=mix(clamp(z.xyz+s()/600,0,1),textureLod(i,e(c-w).xy-x/vec2(1280,720),0).xyz,length(f-c)<.05?.82:.6)*smoothstep(0.,4.,min(a,min(min(abs(a-43),abs(a-75)),abs(a-103))));"
     "}"
   "else"
     " if(m.w<2)"
       "{"
...
//                       "vec4 d=textureLod(i,e(f-w).xy-vec2(c,F)/vec2(1280,720),0);"
         ...
         "else"
           " for(int F=0;F<2;++F)"
             "for(int c=0;c<2;++c)"
               "for(int d=0;d<2;++d)"
                 "z+=texelFetch(l,ivec3(gl_FragCoord.xy*2-1-vec2(1024-exp2(10-r)-d,-c),m.x*2+F),r-1)/8;"
                 
       "}"
     "else if(a > 0)"
     "{"
         "x=abs(gl_FragCoord.xy/vec2(640,360)-1);"
         "z=textureLod(i,gl_FragCoord.xy/vec2(1280,720)+.02,6.5);"
         "z=(z*z+texelFetch(i,ivec2(gl_FragCoord.xy),0))*(2.3-(pow(x.x,8)+pow(x.y,8))*.7);"
         "z=pow(z/(z+1)*2.3,vec4(.8));"
         "a-=120;"
         "x*=exp(-a/7)/(.05+length(x))*4;"
         "for(int F=0;F++<15;)"
           "x+=vec2(cos(x.y*8),sin(x.x*4+x.x*9)-a/8+1)/7,x=abs(x);"
         "z+=exp(-length(x));"
       "}"
   "gl_FragColor=z;"
 "}";
